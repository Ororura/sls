...
- пользовательские временные слоты (`schedule_slots`).
...
Стартовые слоты хранятся в `schedule_slots` для каждого календаря.
...
4. Загрузка пользовательских слотов времени.
5. По дням диапазона:
...
6. Удаление полностью израсходованных элементов пула.
7. Обновление часов у частично израсходованных.
8. Возврат `AutoScheduleResult` с остатком и причинами отказа.
...
### 8.5 Предварительная проверка

`previewAutoSchedule(startDate, endDate)` использует тот же алгоритм, что и `autoSchedule`, но не создаёт занятия и не изменяет пул. Результат содержит планируемое количество занятий, последнюю дату, остаток часов и диагностические причины для нераспределённых строк.

### 8.6 Условия остановки
...
- расширить dry-run до детального отчёта по каждой проверенной дате и слоту.
